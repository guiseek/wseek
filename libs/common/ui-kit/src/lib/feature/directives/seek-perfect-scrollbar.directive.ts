...
import {
  SeekPerfectScrollbarGeometry,
  SeekPerfectScrollbarPosition,
} from './seek-perfect-scrollbar.interfaces';
import { SeekConfigService } from '../config/config.service';

@Directive({
  selector: '[seekPerfectScrollbar]',
})
export class SeekPerfectScrollbarDirective
   ...
   */
  @Input()
  set seekPerfectScrollbarOptions(value) {
    // Merge the options
  ...
  get seekPerfectScrollbarOptions(): any {
    // Return the options
  ...
  ngAfterViewInit(): void {
    ...
    if (this.seekPerfectScrollbarOptions.updateOnRouteChange) {
      this._router.events
        .pipe(
  ...
  _init(): void {
    ...
    this.ps = new PerfectScrollbar(this.elementRef.nativeElement, {
      ...this.seekPerfectScrollbarOptions,
    });
   ...
   * @param prefix
   */
  geometry(prefix: string = 'scroll'): SeekPerfectScrollbarGeometry {
    return new SeekPerfectScrollbarGeometry(
   ...
   * @param absolute
   */
  position(absolute: boolean = false): SeekPerfectScrollbarPosition {
    if (!absolute && this.ps) {
      return new SeekPerfectScrollbarPosition(
        this.ps.reach.x || 0,
        this.ps.reach.y || 0
      );
    } else {
